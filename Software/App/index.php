...
// --- DEVICE CONTROL: OVERRIDE (AUTO/ON/OFF) ---
elseif (preg_match('#^/api/device/([0-9]+)/control$#', $path, $matches) && $method === 'POST') {
    if (!isset($_SESSION['user_id'])) send_json(['status' => 'error', 'message' => 'Nie ste prihlaseny'], 401);
    $device_id = $matches[1];
    $data = get_json_input();
    ...
    if ($action !== 'override' || !in_array($value, ['AUTO', 'ON', 'OFF'])) {
        send_json(['status' => 'error', 'message' => 'Neplatný príkaz. Povolené: action=override, value=AUTO|ON|OFF'], 400);
    }
    
    // Over ze patri userovi
    $stmt = $pdo->prepare("SELECT id, serial_number FROM devices WHERE id = ? AND user_id = ?");
    $stmt->execute([$device_id, $_SESSION['user_id']]);
    $device = $stmt->fetch();
    if (!$device) send_json(['status' => 'error', 'message' => 'Zariadenie nenajdene'], 404);
    
    // Uloz do DB
    $pdo->prepare("UPDATE devices SET manual_override = ? WHERE id = ?")->execute([$value, $device_id]);
    
    // Posli prikaz na CM5
    $config_data = json_encode(['action' => 'update_settings', 'manual_override' => $value]);
    $pdo->prepare("INSERT INTO cm5_config (serial_number, config_json, status) VALUES (?, ?, 'pending')")
        ->execute([$device['serial_number'], $config_data]);
    
    send_json(['status' => 'success', 'manual_override' => $value, 'message' => "Režim zmenený na $value"]);
}

// --- DEVICE CONTROL: AI MODEL SELECTION ---
elseif (preg_match('#^/api/device/([0-9]+)/model$#', $path, $matches) && $method === 'POST') {
    ...
    if (!in_array($model_id, ['1', '2', '3', '4', '5', 'AI'])) {
        send_json(['status' => 'error', 'message' => 'Neplatný model. Povolené: 1-5 alebo AI'], 400);
    }
    ...
    // Uloz do DB
    $pdo->prepare("UPDATE devices SET active_model_id = ? WHERE id = ?")->execute([$model_id, $device_id]);
    
    // Posli prikaz na CM5
    $config_data = json_encode(['action' => 'update_settings', 'active_model_id' => $model_id]);
    $pdo->prepare("INSERT INTO cm5_config (serial_number, config_json, status) VALUES (?, ?, 'pending')")
        ->execute([$device['serial_number'], $config_data]);
    
    send_json(['status' => 'success', 'model_id' => $model_id, 'message' => "AI model zmenený na $model_id"]);
}

// --- INVERTER POWER: OVERRIDE (ON/OFF/AUTO) cez terminál ---
elseif ($path === '/api/system/inverter-power' && $method === 'POST') {
    if (!isset($_SESSION['user_id'])) send_json(['status' => 'error', 'message' => 'Nie ste prihlaseny'], 401);
    $data = get_json_input();
    $power_status = strtoupper(trim($data['power_status'] ?? ''));
    
    if (!in_array($power_status, ['ON', 'OFF', 'AUTO'])) {
        send_json(['status' => 'error', 'message' => 'Neplatný stav. Povolené: ON, OFF, AUTO'], 400);
    }
    
    // Ziskaj prve zariadenie usera (terminal v admin-device nezadava device_id)
    $stmt = $pdo->prepare("SELECT id, serial_number FROM devices WHERE user_id = ? ORDER BY id ASC LIMIT 1");
    $stmt->execute([$_SESSION['user_id']]);
    $device = $stmt->fetch();
    if (!$device) send_json(['status' => 'error', 'message' => 'Zariadenie nenajdene'], 404);
    
    $pdo->prepare("UPDATE devices SET manual_override = ? WHERE id = ?")->execute([$power_status, $device['id']]);
    
    $config_data = json_encode(['action' => 'update_settings', 'manual_override' => $power_status]);
    $pdo->prepare("INSERT INTO cm5_config (serial_number, config_json, status) VALUES (?, ?, 'pending')")
        ->execute([$device['serial_number'], $config_data]);
    
    send_json(['status' => 'success', 'power_status' => $power_status, 'message' => "Režim zmenený na $power_status"]);
}
