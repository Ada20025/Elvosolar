...
# 8. Git pull priamo v hardware adresári (ak existuje)
echo ""
echo "🔄 [8/8] Git pull + reštart..."
if [ -d "$HARDWARE_DIR/.git" ]; then
    cd "$HARDWARE_DIR"
    git fetch origin "$BRANCH" 2>/dev/null || true
    git reset --hard "origin/$BRANCH" 2>/dev/null || true
    echo "✅ Git pull hotový: $(git log --oneline -1)"
    
    # Reštart služby
    if systemctl is-active --quiet elvocontrol 2>/dev/null; then
        sudo systemctl restart elvocontrol
        echo "✅ Služba elvocontrol reštartovaná"
    elif systemctl is-active --quiet elvosolar 2>/dev/null; then
        sudo systemctl restart elvosolar
        echo "✅ Služba elvosolar reštartovaná"
    else
        echo "⚠️  Žiadna systemd služba nenájdená. Spusti ručne."
    fi
else
    echo "⚠️  Git repozitár nenájdený v $HARDWARE_DIR, preskakujem"
fi
