...
class SolarBackgroundService:
    ...
    def read_tcp_device(self, dev, dev_config):
        ...
        # Citanie registrov
        reg_p_ac = dev_config.get('reg_p_ac', 40525)
        reg_soc = dev_config.get('reg_soc', 40515)
        ...
        for sid in range(1, 33):
            data_val = self.tcp_read_holding_registers(device_id, sid, 40525, 2)
            if data_val and (data_val[0] > 0 or (len(data_val) > 1 and data_val[1] > 0)):
                if len(data_val) >= 2:
        ...
        try:
            ...
            if rs485_devices:
                ...
                for dev in rs485_devices:
                    ...
                    if cfg:
                        reg_p_ac = cfg.get('reg_p_ac', 40525)
                        reg_soc = cfg.get('reg_soc', 40515)
