...
def spusti_regulacnu_logiku(model_id: str, cena_aktualna: float, stats: dict, soc: float, core, slave_id: int) -> str:
    ...
    elif porovnanie == "predaj_do_siete":
        # Model 2: Predaj do siete - menič ON keď cena > priemer (predáva za vysokú cenu)
        if cena_aktualna > avg:
            vynut_on = True  # Predávaj keď je drahé
        elif cena_aktualna <= 0:
            vynut_on = False  # Nepredávaj keď je záporná cena
        else:
            vynut_on = False  # Pri normálnej cene - necháme na AI
