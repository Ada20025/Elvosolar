...
CLOUD_SERVER_URL = "https://adamdz.alwaysdata.net"
...
class HolidayModeRequest(BaseModel):
    enabled: bool
    until: str                        # Formát: "YYYY-MM-DD" alebo "YYYY-MM-DD HH:MM:SS"
    preheat_hours: int = 6            # Predstih spustenia prípravy v hodinách
    target_temp: float = 22.0         # Cieľová teplota kúrenia pri príchode
    target_boiler: float = 50.0       # Cieľová teplota vody pri príchode
...
def get_holiday_mode_status():
    """Vráti kompletné informácie o stave a konfigurácii dovolenky."""
    try:
        ...
        return {
            ...
            "holiday_state": holiday_state,  # 'INACTIVE', 'STANDBY', 'PREPARATION'
...
def configure_holiday_mode(data: HolidayModeRequest):
    """Zapne dovolenkový režim a naplánuje čas pred-návratového nahrievania."""
    try:
        ...
        if data.enabled and until_str:
            try:
                ...
                try:
                    ...
                    raise HTTPException(status_code=400, detail="Neplatný formát príchodu. Použite YYYY-MM-DD alebo YYYY-MM-DD HH:MM:SS")
        ...
        # Uloženie konfigurácie do lokálnej DB
        ...
        # Okamžitý prepočet energetického plánu s ohľadom na dovolenku
        bg_service.ai_service.recalculate_plan()
        
        status_label = "AKTIVOVANÝ" if data.enabled else "DEAKTIVOVANÝ"
        if data.enabled:
            notif_msg = f"Dovolenka nastavená do {until_str}. Komfortná príprava začne {data.preheat_hours} hodín pred vaším príchodom."
        else:
            notif_msg = "Dovolenkový režim ukončený. Prechod na štandardné riadenie vlastnej spotreby."

        bg_service.ai_service.create_notification("Dovolenkový režim", notif_msg, "success" if data.enabled else "info", "SYSTEM")
        log_message(f"Dovolenkový režim bol {status_label.lower()}. Ciele: Kúrenie {data.target_temp}°C, Voda {data.target_boiler}°C.")
        
        return {"status": "success", "message": f"Dovolenkový režim bol úspešne nastavený."}
    except Exception as e:
...
def api_system_discover():
    """Vyhľadá pripojené Modbus stanice na kanáli CH1 (/dev/ttyAMA3)."""
    conn = None
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        ...
        row = cursor.fetchone()
        if not row:
            return {"status": "error", "message": "Chýba výber striedača v nastaveniach."}
        brand_id, cat_id, model_id = row
        
        cfg = DEVICE_DB.get(brand_id, {}).get('kategorie', {}).get(cat_id, {}).get('modely', {}).get(model_id)
        if not cfg:
            return {"status": "error", "message": "Neplatná konfigurácia vybraného modelu."}
        
        test_reg = cfg.get('reg_p_ac', 32080)
        ...
        if not os.path.exists(target_port):
            bg_service.paused = False
            return {"status": "error", "message": f"Fyzický port {target_port} (CH1) nie je v systéme dostupný."}
        ...
        with bg_service.lock:
            ...
            for parity in parities_to_test:
                try:
                    ser = serial.Serial(port=target_port, baudrate=cfg.get('baud', 9600), parity=parity, timeout=0.05)
                    for slave_id in range(1, 33):
                        found = bg_service.ping_slave(ser, slave_id, test_reg)
                        if not found and reg_soc:
                            found = bg_service.ping_slave(ser, slave_id, reg_soc)
                        if found:
        ...
        if not discovered_slaves:
            return {"status": "error", "message": "Zbernica neodpovedá. Skontrolujte zapojenie vodičov A/B.", "discovered_count": 0, "slaves": []}
...
# ==============================================================================
# SYSTÉMOVÉ REST API ENDPOINTY (WIFI, PROVISIONING, REBOOT)
# ==============================================================================
...
def get_dashboard_data(slave_id: int):
    """Zabezpečuje rýchle dopytovanie pre real-time dashboard vrátane prenosu dovolenky."""
    try:
...
def execute_terminal_command(data: TerminalCommandRequest):
    """Bezpečne vykoná systémový príkaz na doske Waveshare CM5."""
    if data.token != "cm5_master_secure_9921":
        return {"status": "error", "message": "Prístup zamietnutý - Neplatný bezpečnostný kľúč."}
    ...
    if cmd == "restart_services":
        ...
        bg_service.ser = None
        bg_service.paused = False
        return {"status": "success", "message": "Zbernicové rozhrania úspešne reštartované."}
        
    elif cmd == "reboot_pi":
        threading.Thread(target=lambda: (time.sleep(1), os.system("sudo reboot"))).start()
        return {"status": "success", "message": "Reštartujem jednotku CM5..."}
        
    elif cmd == "clear_logs":
        SYSTEM_LOGS.clear()
        bg_service.terminal_logs.clear()
        return {"status": "success", "message": "Súbory logov prečistené."}
...
def serve_portal():
    """Zobrazí lokálneho sprievodcu claimovania alebo presmeruje do Alwaysdata cloudu."""
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT value FROM system_settings WHERE key = 'is_claimed'")
    row = cursor.fetchone()
...
def api_reset_system():
    """Továrenské vymazanie nastavení jednotky (Factory wipe)."""
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("DELETE FROM system_settings WHERE key = 'is_claimed'")
