...
class SmartMeterService:
    ...
    MODE_ZERO = "ZERO"                    # Pri nule: FVE presne pokrýva spotrebu domu
    MODE_PLUS = "PLUS"                    # Pri pluse: prebytok → batéria/sieť
    MODE_CUSTOM = "CUSTOM"                # Vlastný režim s konfiguráciou
    ...
    def __init__(self, bg_service=None):
        ...
        # Riadiaci režim
        self.control_mode = self.MODE_SMART  # UNLIMITED, SELF_CONSUMPTION, SMART, ZERO, PLUS, CUSTOM
        
        # CUSTOM režim nastavenia
        self.target_consumption_w = 0.0     # Cieľová spotreba domu [W]
        self.surplus_action = "CHARGE_BATTERY"  # CHARGE_BATTERY, EXPORT_GRID, PRODUCE_HOUSE, STOP
        self.zero_action = "PRODUCE_HOUSE"      # PRODUCE_HOUSE, CHARGE_BATTERY, EXPORT_GRID, STOP
        self.grid_export_limit_w = 0.0      # Limit vývozu do siete [W], 0 = bez limitu
        self.battery_priority = "SMART"     # SMART, MAX_CHARGE, MIN_DISCHARGE, PRESERVE
    ...
    def _load_config(self):
        ...
        try:
            ...
            for r in rows:
                ...
                elif k == 'target_consumption_w': self.target_consumption_w = float(v)
                elif k == 'surplus_action': self.surplus_action = v
                elif k == 'zero_action': self.zero_action = v
                elif k == 'grid_export_limit_w': self.grid_export_limit_w = float(v)
                elif k == 'battery_priority': self.battery_priority = v
    ...
    def get_config(self) -> dict:
        ...
        return {
            ...
            'target_consumption_w': self.target_consumption_w,
    ...
    def should_inverter_run(self, okte_price: float = 80.0, avg_price: float = 80.0, 
        ...
        elif mode == self.MODE_ZERO:
            # PRI NULE: Keď sieť = 0W, FVE produkuje presne čo dom potrebuje
            # Ak dom spotrebováva 0W, FVE sa zastaví
            # Ak dom spotrebováva X W, FVE produkuje X W
            if consumption <= 10:
                return {
                    'target_on': False,
                    'reason': f'ZERO: Spotreba {consumption:.0f}W ≈ 0 - menič vypnutý.',
                    'mode': 'ZERO_IDLE',
                    'limit_w': 0
                }
            else:
                return {
                    'target_on': True,
                    'reason': f'ZERO: FVE produkuje presne {consumption:.0f}W pre dom.',
                    'mode': 'ZERO_MATCH',
                    'limit_w': consumption
                }
        
        elif mode == self.MODE_PLUS:
            # PRI PLUSE: Keď je prebytok, všetko do batérie/siete
            # Keď je mínus, vybíja sa batéria
            if surplus > 50:
                # Prebytok - nabíjame batériu alebo vyvádzame
                return {
                    'target_on': True,
                    'reason': f'PLUS: Prebytok {surplus:.0f}W - nabíjanie/vývoz.',
                    'mode': 'PLUS_SURPLUS',
                    'limit_w': -1
                }
            elif consumption > 100:
                # Spotreba - vybíjame batériu
                return {
                    'target_on': True,
                    'reason': f'PLUS: Spotreba {consumption:.0f}W - vybíjanie batérie.',
                    'mode': 'PLUS_DISCHARGE',
                    'limit_w': consumption
                }
            else:
                return {
                    'target_on': False,
                    'reason': f'PLUS: Minimálna spotreba - menič standby.',
                    'mode': 'PLUS_STANDBY',
                    'limit_w': 0
                }
        
        elif mode == self.MODE_CUSTOM:
            # VLASTNÝ REŽIM: Podľa používateľských nastavení
            target = self.target_consumption_w
            
            # Akcia pri NULE (spotreba ≈ 0)
            if consumption <= 10:
                actions = {
                    'PRODUCE_HOUSE': {'on': False, 'reason': 'CUSTOM/ZERO: Spotreba≈0, produkcia zastavená.', 'limit': 0},
                    'CHARGE_BATTERY': {'on': True, 'reason': 'CUSTOM/ZERO: Spotreba≈0, nabíjanie batérie.', 'limit': -1},
                    'EXPORT_GRID': {'on': True, 'reason': 'CUSTOM/ZERO: Spotreba≈0, vývoz do siete.', 'limit': -1},
                    'STOP': {'on': False, 'reason': 'CUSTOM/ZERO: Spotreba≈0, menič vypnutý.', 'limit': 0},
                }
                a = actions.get(self.zero_action, actions['PRODUCE_HOUSE'])
                return {'target_on': a['on'], 'reason': a['reason'], 'mode': 'CUSTOM_ZERO', 'limit_w': a['limit']}
            
            # Akcia pri PLUSE (prebytok > 0)
            elif surplus > 50:
                actions = {
                    'CHARGE_BATTERY': {'on': True, 'reason': f'CUSTOM/PLUS: Prebytok {surplus:.0f}W, nabíjanie batérie.', 'limit': -1},
                    'EXPORT_GRID': {'on': True, 'reason': f'CUSTOM/PLUS: Prebytok {surplus:.0f}W, vývoz do siete.', 'limit': -1},
                    'PRODUCE_HOUSE': {'on': True, 'reason': f'CUSTOM/PLUS: Prebytok {surplus:.0f}W, produkcia pre dom.', 'limit': consumption},
                    'STOP': {'on': False, 'reason': f'CUSTOM/PLUS: Prebytok {surplus:.0f}W, menič vypnutý.', 'limit': 0},
                }
                a = actions.get(self.surplus_action, actions['CHARGE_BATTERY'])
                return {'target_on': a['on'], 'reason': a['reason'], 'mode': 'CUSTOM_PLUS', 'limit_w': a['limit']}
            
            # Štandardná spotreba - produkujeme cieľovú hodnotu
            else:
                return {
                    'target_on': True,
                    'reason': f'CUSTOM: Spotreba {consumption:.0f}W, cieľ {target:.0f}W.',
                    'mode': 'CUSTOM_NORMAL',
                    'limit_w': max(target, consumption)
                }
